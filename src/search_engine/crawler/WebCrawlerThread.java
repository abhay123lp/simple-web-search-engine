...
public class WebCrawlerThread extends WebCrawler {	
	...
	private final int DEFAULT_PORT_NUMBER_USED = 80; // port number for web server
	private final int CRAWLING_RATE = 500; // in miliseconds, delay between 2 consecutive crawling
	 ...
	 * Check the depth: if reach maximum depth then quit
	 * Else try load the starting url and report
	 */
	public void run() {
		if (depth < maxDepth) {
			...
			if (responseTime != CRAWLING_IO_ERROR && responseTime != CRAWLING_TIMEOUT_ERROR) {
				...
				// Check for redirection and check crawlers in the redirected page
	...
	/**
	 * Try to load the page and store all the page content inside the pageContent field
	 * At the same time measure the time taken to load the full page
	 * @param link: the url link of the page
	 * @return the time taken to load the full page. INVALID_NUMBER is returned if an error occured
	 */
	private long LoadPageAndGetResponseTime (String link) {
		try {
			...
			do {
				...
				if (responseTime == -1) {
					responseTime = System.currentTimeMillis(); // response time is taken when the first few bytes are available in the reader stream
				}
				
				if ((newLine != null) && (newLine.compareTo("") == 0)) {
					isReadingResponseReader = false; // now reading to the package content
				}
			...
			// TODO: small Hack to break whenever reading </html>
			// consider fixing problem when server "forget" to close the input stream
	...
	/**
	 * Scan through the HTML Document, retrieve all the href field in a tag
	 * Then send crawlers into that link.
	 * @param pageContent: the HTML Document of the page
	 */
	private void SendCrawlersInPage (Document pageContent) {
		Elements links = pageContent.getElementsByTag("a"); // Get list of a tags
	...
	/**
	 * Check the header for redirection
	 * If recieve a 3xx, get the link from LOCATION field and follow that link
	 * @param headerContent
	 */
	...
	/**
	 * Send crawler to the domain of the link provided
	 * Don't send if the domain have already visited or the max depth has been reached 
	 * @param link
	 */
	private void SendCrawlerIntoLink(String link) {
		link = ReparseLink (link);
		if (WebCrawler.CheckAndAddLink(link)) { // check whether the link has been crawled
			if (depth < maxDepth-1) {
				// instantiate a new crawler, wait a while then send it into the retrieved link
				Timer crawlingTimer = new Timer ("crawlingTimer", false); // false = not Daemon: the application will wait until the children crawlers finish before terminating
	...
	/**
	 * Re-parse Method
	 * This method will extract the domain part of the link and remove other.
	...
	private String ReparseLink (String link) {
	    ...
	    String domainName = domain == null ? "" : domain; // if null then return empty string
	    return domainName;
	}
}
